...
def main():
    ...
    for file in glob.glob(cfg.PREDICTION + "*.txt"):
        ...
        if (len(pr_yolo)!=0):
            ...
            # print('With Hungarian:')
            # print(cost)
            Eucl_err_h.append(cost)

            # print('With Simple estimate:')
            Eucl_err_s.append(math.sqrt(pow((x_yolo - gt_xyz[0]), 2) + pow((y_yolo - gt_xyz[1]), 2) + pow((z_yolo - gt_xyz[2]), 2)))
